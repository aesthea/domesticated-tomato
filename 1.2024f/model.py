...
def edet(input_shape = (256, 256, 3), num_classes = 1000, detection_region = 5, dropout = 0.2, backbone = "B0"):
    ...
    f1 = biFPN(bn_activation, ACTIVATION = tf.keras.layers.LeakyReLU(), sequence = 1)
    f2 = biFPN(f1, ACTIVATION = tf.keras.layers.LeakyReLU(), sequence = 2)
    f3 = biFPN(f2, ACTIVATION = tf.keras.layers.LeakyReLU(), sequence = 3)
...
def biFPN(layers, KERNELS = 3, ACTIVATION = "relu", dropout = 0.2, sequence = 1):
    f1_block = layers
    f2_block = []
    f3_block = []
    f4_block = []
    ...
    for i, j in enumerate(f1_block):
        if i == 0 or i == len(f1_block) - 1:
            ...
            x = j
            y = tf.keras.layers.Conv2D(filters=j.shape[-1], kernel_size=KERNELS, strides=strides, padding='same', activation = ACTIVATION, name = "fpn_%02d_f2_%02d_Conv2D_y" % (sequence, i - 1))(c)
            y = tf.keras.layers.BatchNormalization(name = "fpn_%02d_f2_%02d_BN" % (sequence, i - 1))(y)
            y = tf.keras.layers.Activation(ACTIVATION, name = "fpn_%02d_f2_%02d_Activation" % (sequence, i - 1))(y)             
    ...
    for i, j in enumerate(f2_block):
        ...
        #F3_3
            ...
            x = j
            y = tf.keras.layers.Conv2D(filters=j.shape[-1], kernel_size=KERNELS, strides=strides, padding='same', activation = ACTIVATION, name = "fpn_%02d_f3_%02d_Conv2D_y" % (sequence, i - 1))(f2_block[i - 1])
            y = tf.keras.layers.BatchNormalization(name = "fpn_%02d_f3_%02d_BN" % (sequence, i - 1))(y)
            y = tf.keras.layers.Activation(ACTIVATION, name = "fpn_%02d_f3_%02d_Activation" % (sequence, i - 1))(y)            
            c = tf.keras.layers.Add(name = "fpn_%02d_f3_%02d_ADD" % (sequence, i))([x, y])
            f3_block.append(c)
    ...
    for ri, j in enumerate(f3_block):
        ...
        #F4_2, F4_1, F4_0
            ...
            x = j
            y = tf.keras.layers.Conv2DTranspose(j.shape[-1], KERNELS, strides=(strides, strides), padding="same", activation = ACTIVATION, name = "fpn_%02d_f4_%02d_Conv2D" % (sequence, i + 1))(c)
    ...
    for i, j in enumerate(f4_block):
        ...
        x = tf.keras.layers.Activation(ACTIVATION, name = "fpn_%02d_f5_%02d_Activation" % (sequence, i))(x)
        f5_block.append(x)
