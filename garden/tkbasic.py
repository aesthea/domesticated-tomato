...
class widget:
    def __init__(self):
        self.root = Tk()
        self.root.title("HARR VOTT 1.2024e")
...
##        self.top_activate.pack(fill = BOTH, expand = True)
        ...
        self.label(self.frame3, 600, 25, 2, 0, "Backbone", columnspan = 2)
        self.backbone_frame = Frame(master = self.frame3, width = 600, height = 25)
...
##        self.fpn_radio1["state"] = "disabled"
##        self.fpn_radio2["state"] = "disabled"
...
##        self.normalization_checkbutton["state"] = "disabled"
        ...
        self.objects["random_drop"]["input"]["state"] = "disabled"
        self.objects["overlap"]["input"]["state"] = "disabled"
        self.objects["huber"]["input"]["state"] = "disabled"
...
##                reload_ML = True
            if self.DET_MODEL.NULL_SKIP != self.value_type(self.inputs["nullskip"], float):
                self.DET_MODEL.NULL_SKIP = self.value_type(self.inputs["nullskip"], float)
                if 'sanity_check_sample' in dir(self.DET_MODEL.model):
                    del(self.DET_MODEL.model.sanity_check_sample)

            if self.DET_MODEL.BATCH_SIZE != self.value_type(self.inputs["batchsize"], int):
                self.DET_MODEL.BATCH_SIZE = self.value_type(self.inputs["batchsize"], int)
                if 'sanity_check_sample' in dir(self.DET_MODEL.model):
                    del(self.DET_MODEL.model.sanity_check_sample)
    ...
    def generator_check(self):
        ...
        if 'sanity_check_sample' not in dir(self.DET_MODEL.model):
            if os.path.isdir(self.data["savefile"]):
                if len(os.listdir(self.data["savefile"])) > 10:
